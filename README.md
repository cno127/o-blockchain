Here is the first draft for the architecture of the O coin and how we envision it. This is not a final version but just a base for discussion and feedback on how would the O coins work. This has been mainly generated by AI after long discussion regarding the specifications and requirements of the project and after considering different options and prototype. Other option considered was to create one fork per currency and keep the structure mono currency, but it requires lots of maintenance while being an interesting model in terms of scalability since we divide the load of transactions by volume per currency per blockchain. Other option considered was also using other frameworks that are multi currency ready but those solutions are less flexible and more dependable.
